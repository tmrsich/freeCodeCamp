...
int[] testData2 = new int[] { 6, 10, 17, 20, 31, 36, 40, 41 };
int[] testData = new int[] { 6, 10, 17, 20, 31, 36, 40, 41 };
int testCounter = 0;

LoadTillEachMorning(registerDailyStartingCash, cashTill);

// This adds up the total value of the cash register, by adding up the type of dollar bill times the quantity of the bill. (i.e. $5 * 20 bills => $100)
...
// Will always be true unless another set of data is inputted and the initialization of the bool is set to false
if (useTestData)
{
...
while (transactions > 0)
{
    ...
    // If using the data from the testData array
    if (useTestData)
    {
...
// Sets each value in the cashTill array at an index position to be equal to the 2nd value in each nested array in the registerDailyStartingCash array
...
// Update to include the creation and throwing of exceptions for "Insufficient Till" and "Undperpayment" issues
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded < 0)
        throw new InvalidOperationException("InvalidOperationException: Not enough money to complete the transaction.");
    ...
    {
        // Was originally taking money out of the cashTill[2] element, which is where the $10 bills are "stored," not the $5 bills. Changed cashTill[2] => cashTill[1]
        cashTill[1]--;
        changeNeeded -= 5;
...
// Method to print out the information on the money currently in the cash register made up of individual bills
...
// Method similar to the one above except it sums the total amount of money in the register
